...
public class Table implements Saveable {
	...
	public void Parse(String text) {
		String[] parts = text.split(CrashedDBsep.regexFormat(CrashedDBsep.sepTableHeadersFromData));
		String[] rawHeaders = parts[0].split(CrashedDBsep.regexFormat(CrashedDBsep.sepTableHeaders));

		for (String header : rawHeaders) {
			addColumn(header);
		}

		String[] rows = parts[1].split(CrashedDBsep.regexFormat(CrashedDBsep.sepRow));
		String[] _values;
		ArrayList<DBvalue> row;

		for (String _row : rows) {
			row = new ArrayList<DBvalue>();
			_values = _row.split(CrashedDBsep.regexFormat(CrashedDBsep.sepValue));
